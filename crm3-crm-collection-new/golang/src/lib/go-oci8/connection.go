...
func (conn *OCI8Conn) ociLobWrite(lobLocator *C.OCILobLocator, data []byte) (n int, err error) {
	byteAmt := C.oraub8(len(data))
	result := C.OCILobWrite2(
		conn.svc,                 //OCISvcCtx          *svchp,
		conn.errHandle,           //OCIError           *errhp,
		lobLocator,               //OCILobLocator      *locp,
		&byteAmt,                 //oraub8          *byteAmtp,
		nil,                      //oraub8          *char_amtp,
		C.oraub8(1),              //oraub8          offset, starting position is 1
		unsafe.Pointer(&data[0]), //void            *bufp,
		C.oraub8(len(data)),
		C.OCI_ONE_PIECE,  //ub1             piece,
		nil,              //void            *ctxp,
		nil,              //OCICallbackLobWrite2 (cbfp)
		0,                //C.ub2(atomic.LoadUint32(&csIDAl32UTF8)), //ub2             csid,
		C.SQLCS_IMPLICIT, //ub1             csfrm );
	//fmt.Printf("C.OCI_NEED_DATA %v, C.OCI_SUCCESS %v\n", C.OCI_NEED_DATA, C.OCI_SUCCESS)
	)
	if result == C.OCI_ERROR {
		return 0, errors.New("ociLobWrite:" + conn.getError(result).Error())
	}
	return int(byteAmt), nil
}

func (conn *OCI8Conn) ociLobCreateTemporary(lobLocator *C.OCILobLocator) error {
	...
	if result != C.OCI_SUCCESS {
		return errors.New("ociLobOpen:" + conn.getError(result).Error())
	}
	return nil
}

func (conn *OCI8Conn) ociLobFreeTemporary(lobLocator *C.OCILobLocator) error {
	...
	C.OCIDescriptorFree(
		unsafe.Pointer(lobLocator), //void     *descp,
