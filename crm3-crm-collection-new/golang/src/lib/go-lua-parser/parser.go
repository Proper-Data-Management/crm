...
func (p *AstNode) set(k string, v interface{}) *AstNode {
	(*p)[k] = v
	return p
}
func (p *AstNode) get(k string) interface{} {
	return (*p)[k]
}
func (p *AstNode) astPush(name string) *AstNode {
	//log.Println("AstNode.astPush")
	n := &AstNode{}
	(*n)["n"] = name
	a, ok := (*p)["n"].([]*AstNode)
	if !ok {
		(*p)["z"] = make([]*AstNode, 0, 1)
	}
	(*p)["z"] = append(a, n)
	return n
}
...
func (p *parser) InitAst() {
	//log.Println("parser.InitAst")
	p.astStack = make([]*AstNode, 0, 0)
	p.astStack = append(p.astStack, &AstNode{})
}
func (p *parser) astPush(name string) *AstNode {
	//log.Println("parser.astPush")
	n := p.AstTop().astPush(name)
	p.astStack = append(p.astStack, n)
	return n
}
func (p *parser) astPop() {
	//log.Println("parser.astPop")
	p.astStack = p.astStack[:len(p.astStack)-1]
}
func (p *parser) AstTop() *AstNode {
	l := len(p.astStack)
	if l == 0 {
		panic("zero ast")
	}
	return p.astStack[l-1]
}
...
func (p *parser) expressionStatement() {
	astExp := p.astPush("expressionStatement")
	e := p.suffixedExpression()
	astExp.set("kind", kinds[e.kind])
...
func (l *State) parse(r io.ByteReader, name string) *luaClosure {
	...
	l.push(c)
	l.PushUserData(p)
	l.SetGlobal(PARSER_KEY)
	return c
}
func (l *State) GetParser() *parser {
	l.Global(PARSER_KEY)
	data := l.ToUserData(-1)
	l.Pop(1)
	p, _ := data.(*parser)
