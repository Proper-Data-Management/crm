...
func (o *rawSet) Columns() ([]string, error) {
...
func (o *rawPrepare) readValues(container interface{}, args ...interface{}) (int64, error) {
	var (
		maps  []Params
		lists []ParamsList
		list  ParamsList
	...
	args2 := getFlatParams(nil, args, o.rs.orm.alias.TZ)

	var rs *sql.Rows

	rs, err := o.stmt.Query(args2...)
	if err != nil {
		return 0, err
	}
	...
	for rs.Next() {
		if cnt == 0 {
			...
			colTypes = make(Params)

			tt, err := rs.ColumnTypes()
			if err != nil {
				return 0, err
			}
			...
			for _, tp := range tt {
				st := tp.ScanType()
				if st != nil {
					colTypes[tp.Name()] = tp.DatabaseTypeName()
				}
			}
		...
		case 1:
			...
			for _, i := range indexs {
				...
				if value.Valid {
					...
					if colTypes[cols[i]] == "SQLT_DAT" {
						layout := "2006-01-02T15:04:05-07:00"
						t, _ := time.Parse(layout, value.String)
						params[strings.ToLower(cols[i])] = t.Format("2006-01-02 15:04:05")
					} else {
						params[strings.ToLower(cols[i])] = value.String
					}
				} else {
					params[strings.ToLower(cols[i])] = nil
				}
			}
...
// query data to []map[string]interface
func (o *rawPrepare) Values(container *[]Params, args ...interface{}) (int64, error) {
	if o.closed {
		return 0, ErrStmtClosed
	}
	return o.readValues(container, args...)
}
